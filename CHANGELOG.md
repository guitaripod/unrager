...
- **`unrager snapshot` subcommand.** New headless renderer that builds a real `App` against your X session, fetches whatever a given view needs (home / detail / notifications / profile / search / help / ask / compose), eagerly decodes every visible avatar and media image to RGBA in memory, then runs a single `ui::draw` pass against a ratatui `TestBackend` and rasterizes the resulting buffer to PNG — with kitty image placeholders detected by sweeping the buffer and composited from the in-memory map. Used to regenerate the site carousel deterministically: `unrager snapshot --view detail --out detail.png`. Output dimensions follow `--cols × --cell-w` by `--rows × --cell-h` (defaults: 160 × 12 by 48 × 25 → 1920×1200). Bypasses the live TUI's async event loop, so the same command produces the same image every time the data is the same.
- **Carousel screenshots regenerated.** Every PNG on `unrager.com` now ships with author avatars, profile-header avatars, and notification-actor avatars baked in — the chip-less postcards predating `feat(tui): inline author-avatar chips in feed rows` are gone.
