...
The native clients talk to `unrager serve` over HTTP/SSE (pair with Tailscale). They live in the repo alongside the Rust crates:
...
- `macos/` — AppKit Mac app (macOS 26, `NSGlassEffectView`), same `UnragerKit`, same feature set. QA hooks mirror iOS: `UNRAGER_SERVER` env points it at a server, and (DEBUG) `UNRAGER_SELFSHOT=<path>` renders the window to a PNG in-process — an agent can screenshot the Mac app this way without the permission-gated Screen Recording API (`screencapture` fails with "could not create image from window" when Screen Recording isn't granted).
