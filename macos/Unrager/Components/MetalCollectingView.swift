...
/// A full-window, Metal-rendered loading state for the rage-filter's
...
///
/// Driven by `MTKView`'s own display link at ~60fps; paused whenever the view
/// is hidden or off-window so it draws zero frames in the background. When
/// `MTLCreateSystemDefaultDevice()` returns nil (CI / headless), `isAvailable`
/// is false and the caller falls back to the spinner + label.
final class MetalCollectingView: NSView {
    /// False when no Metal device exists. The caller keeps the legacy spinner +
    /// label path alive for this case.
    let isAvailable: Bool

    private var metalView: MTKView?
    private var renderer: CollectingRenderer?
    private let caption = NSTextField(labelWithString: "")
    private let titleLabel = NSTextField(labelWithString: "Collecting your feed")
    ...
    private func configureCaption() {
        ...
        let stack = NSStackView(views: [titleLabel, caption])
        ...
        NSLayoutConstraint.activate([
            ...
            stack.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -DesignSystem.Spacing.xxl * 2),
    ...
    func update(collected: Int, target: Int) {
        ...
        renderer?.targetProgress = min(1, Float(collected) / Float(denom))
        caption.stringValue = "Collecting tweets… \(collected)/\(denom)"
    }
    ...
    /// Stops drawing entirely — no frames — when hidden or removed.
    func stop() {
    ...
    private func syncAppearance() {
        ...
        effectiveAppearance.performAsCurrentDrawingAppearance {
            accent = DesignSystem.Color.accent.simd3
            background = DesignSystem.Color.background.simd3
        }
        renderer?.setPalette(accent: accent, background: background, isDark: isDark)
    }
}
...
private final class CollectingRenderer: NSObject, MTKViewDelegate {
    ...
    func setPalette(accent: SIMD3<Float>, background: SIMD3<Float>, isDark: Bool) {
...
private extension NSColor {
    var simd3: SIMD3<Float> {
        guard let rgb = usingColorSpace(.sRGB) else { return SIMD3<Float>(0, 0, 0) }
...
/// with the iOS client (same source string).
