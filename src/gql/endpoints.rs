...
pub fn home_timeline_variables(count: u32, cursor: Option<&str>, seen_ids: &[&str]) -> Value {
    let mut vars = json!({
        "count": count,
        "includePromotedContent": false,
...
pub fn notifications_timeline_variables(count: u32, cursor: Option<&str>) -> Value {
    let mut vars = json!({
        "timeline_type": "All",
        "count": count,
    });
...
pub fn notifications_timeline_features() -> Value {
    json!({
        "rweb_video_screen_enabled": false,
...
pub fn create_tweet_features() -> Value {
    json!({
        "premium_content_api_read_enabled": false,
        "communities_web_enable_tweet_community_results_fetch": true,
        "c9s_tweet_anatomy_moderator_badge_enabled": true,
