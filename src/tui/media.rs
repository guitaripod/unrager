...
impl MediaRegistry {
    ...
    /// Queues download + transmit for an arbitrary card thumbnail URL
    /// discovered after the tweet's media list was built (currently
    /// song.link thumbnails resolved asynchronously). Reuses the regular
    /// media cache so repeat views are instant.
    pub fn ensure_card_thumbnail_url(&mut self, url: &str, tx: &EventTx) {
        if !self.supported() || url.is_empty() {
            return;
        }
        if self.entries.contains_key(url) {
        ...
        let is_kitty = self.is_kitty();
        let cache_path = self.media_cache_path(url);
        self.insert_entry(url.to_string(), MediaEntry::Loading);
        let url = url.to_string();
        let sem = self.semaphore.clone();
        let tx = tx.clone();
