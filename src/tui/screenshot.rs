...
pub struct TweetBlock {
    ...
    /// Color emoji PNGs decoded to RGBA, keyed by Twemoji filename stem
    /// (e.g. `1f525` for 🔥, `1f1fa-1f1f8` for the US flag). Whenever
    /// `paint_buffer` encounters an emoji grapheme it looks up the
    /// corresponding image and composites it over the cell(s). Stems
    /// absent from this map fall through to the outline font (a
    /// monochrome glyph or, for true color emoji, nothing — better than a
    /// stale letter pair). The map is populated lazily by the caller
    /// (`tui::emoji_cache`), which fetches missing PNGs from the
    /// maintained Twemoji CDN and caches them under
    /// `~/.cache/unrager/emoji/`.
    pub emoji: HashMap<String, RgbaImage>,
}
...
struct PreparedBlock {
    ...
    emoji: HashMap<String, RgbaImage>,
}
...
fn paint_buffer(
    ...
    for y in 0..rows {
        for x in 0..buf.area.width {
            ...
            // Emoji grapheme → color PNG. ab_glyph can't render color
            // emoji tables or apply ligature shaping, so the bundled
            // monospace fonts would draw the codepoints as tofu. Composite
            // the cached Twemoji PNG when we have it, sized to the
            // grapheme's display width. On a miss (offline, 404, not yet
            // cached) fall through to the outline font rather than leave a
            // hole — a monochrome glyph beats a gap.
            if crate::tui::emoji_cache::is_emoji_grapheme(sym) {
                let stem = crate::tui::emoji_cache::twemoji_stem(sym);
                if let Some(img) = emoji.get(&stem) {
                    let slot_w = emoji_slot_cells(sym) * grid.cell_w;
                    composite_emoji(canvas, img, px_x, px_y, slot_w, grid.line_h);
                    continue;
                }
            }
...
/// Number of grid cells an emoji grapheme occupies. `UnicodeWidthStr::width`
/// collapses a well-formed cluster atomically (2 for most emoji, ZWJ
/// sequences, skin-tone modifiers, keycaps, and VS16; 1 for text
/// presentation), so summing per-char would overshoot. Clamped to ≥1.
fn emoji_slot_cells(grapheme: &str) -> u32 {
    unicode_width::UnicodeWidthStr::width(grapheme).max(1) as u32
}

/// Resizes the (square 72×72) Twemoji PNG to fit the emoji's cell slot and
/// alpha-blends it onto the canvas. The image is aspect-preserved and
/// centered in the slot; for a square source in a 2-cell slot, height fills
/// the line and it centers horizontally.
fn composite_emoji(
    canvas: &mut RgbaImage,
    img: &RgbaImage,
    px_x: u32,
    ...
    let resized = imageops::resize(img, dst_w, dst_h, imageops::FilterType::Lanczos3);
...
mod tests {
    ...
    /// End-to-end emoji path against the live CDN. Fetches real Twemoji
    /// PNGs and composites them, dumping a card to the temp dir for visual
    /// inspection. Ignored by default (needs network).
    #[tokio::test]
    #[ignore = "network: fetches real Twemoji PNGs"]
    async fn render_with_real_emoji_writes_png() {
        let accent = Style::default().fg(Color::Rgb(0x1d, 0x9b, 0xf0));
        let lines = vec![
            Line::from(vec![Span::styled("@emoji_test", accent)]),
            Line::from("  fire 🔥 heart ❤️ thumbs 👍🏿 flag 🇯🇵 ☺️"),
            Line::from("  family 👨‍👩‍👧‍👦 keycap 1️⃣ smile 😀 rocket 🚀 ✨"),
        ];
        let stems = crate::tui::emoji_cache::extract_emoji_stems(&lines);
        assert!(!stems.is_empty(), "should detect emoji stems");
        let emoji = crate::tui::emoji_cache::load_emoji(stems).await;
        assert!(!emoji.is_empty(), "should fetch at least one emoji PNG");
        let cap = render(RenderArgs {
            tweet_id: "emoji".into(),
        ...
        let path = std::env::temp_dir().join("unrager-emoji-test.png");
